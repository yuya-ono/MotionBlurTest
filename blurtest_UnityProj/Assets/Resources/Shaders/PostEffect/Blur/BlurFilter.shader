Shader "Custom/BlurFilter" {
	Properties {
		_MainTex ("Base (RGB)", 2D) = "white" {}
		_Volume("Blur Power", float) = 1.0
	}
...
//				float2 uv[4]: TEXCOORD;
			...
			float4 frag(v2f i):COLOR{
				//texel size
				float offX = _MainTex_TexelSize.x*2.0f * _Volume;
				float offY = _MainTex_TexelSize.y*2.0f * _Volume;
				float4 col = tex2D(_MainTex, i.texcoord) * 0.1f;
				float4 col1 = tex2D(_MainTex, i.texcoord + float2(offX * 1.0f, 0)) * 0.15f;
				float4 col2 = tex2D(_MainTex, i.texcoord + float2(offX * 2.0f, 0)) * 0.1f;
				float4 col3 = tex2D(_MainTex, i.texcoord + float2(offX * -1.0f, 0)) * 0.1f;
				float4 col4 = tex2D(_MainTex, i.texcoord + float2(offX * -2.0f, 0)) * 0.1f;
				
				float4 col5 = tex2D(_MainTex, i.texcoord + float2(0, offY * 1.0f)) * 0.15f;
				...
				float4 col8 = tex2D(_MainTex, i.texcoord + float2(0, offY * -1.0f)) * 0.1f;
...
////				float offX = _MainTex_TexelSize.x*2.0f;
//				float offY = _MainTex_TexelSize.y*2.0f;
//				float4 col = tex2D(_GrabTexture, i.texcoord) * 0.1f;
//				float4 col1 = tex2D(_GrabTexture, i.texcoord + float2(0, offY * 1.0f)) * 0.1f;
//				float4 col2 = tex2D(_GrabTexture, i.texcoord + float2(0, offY * 2.0f)) * 0.1f;
//				float4 col3 = tex2D(_GrabTexture, i.texcoord + float2(0, offY * 3.0f)) * 0.1f;
//				float4 col4 = tex2D(_GrabTexture, i.texcoord + float2(0, offY * 4.0f)) * 0.1f;
//				float4 col5 = tex2D(_GrabTexture, i.texcoord + float2(0, offY * 5.0f)) * 0.05f;
//				
//				float4 col6 = tex2D(_GrabTexture, i.texcoord + float2(0, offY * -1.0f)) * 0.1f;
//				float4 col7 = tex2D(_GrabTexture, i.texcoord + float2(0, offY * -2.0f)) * 0.1f;
//				float4 col8 = tex2D(_GrabTexture, i.texcoord + float2(0, offY * -3.0f)) * 0.1f;
//				float4 col9 = tex2D(_GrabTexture, i.texcoord + float2(0, offY * -4.0f)) * 0.1f;
//				float4 col10 = tex2D(_GrabTexture, i.texcoord + float2(0, offY * -5.0f)) * 0.05f;
//				
//				col = col + col1 + col2 + col3 + col4 + col5
//				 		  + col6 + col7 + col8 + col9 + col10; 
//				
